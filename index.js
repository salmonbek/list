...
["select of position", ...TypePosition].forEach((typeposition) => {
  filterPosition.innerHTML += `<option value="${typeposition}">${typeposition}</option>`;
});

["select adress", ...City].forEach((city) => {
...
function deletePupil(id) {
  let isConfirm = confirm("Do yo want to delete ?");
  if (isConfirm) {
...
filterPosition.addEventListener("change", function () {
  if (this.value == "Select of Position") {
    getPupils();
  } else {
